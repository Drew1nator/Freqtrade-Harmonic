...
class HarmonicDivergence(IStrategy):
    ...
    buy_params = { 
        ...
        "rocr1_1h": 0.9346,
        "rocr_1h": 0.65666,                       
        "base_nb_candles_buy": 5,  # value loaded from strategy
        "buy_bb_delta": 0.015,  # value loaded from strategy
        "buy_bb_factor": 0.995,  # value loaded from strategy
        "buy_bb_width": 0.035,  # value loaded from strategy   
        ...
        "buy_closedelta": 4.0,  # value loaded from strategy                                                                                                                                                                                 
    ...
    sell_params = {
        ...
        "base_nb_candles_sell": 8,  # value loaded from strategy
    ...
    trailing_stop = False
    trailing_stop_positive = 0.003 #povodne 0.001
    trailing_stop_positive_offset = 0.007 #povodne 0.012
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        inf_tf = '5m'
        informative = self.dp.get_pair_dataframe('BTC/USDC', timeframe=inf_tf)
        ...
        # --- Informative 15m timeframe
        inf_tf_15m = '15m'
        informative_15m = self.dp.get_pair_dataframe(pair=metadata['pair'], timeframe=inf_tf_15m)
        informative_15m['rsi_14_15m'] = ta.RSI(informative_15m, timeperiod=14)
        informative_15m['cmf_15m'] = chaikin_money_flow(informative_15m, 20)
        informative_15m['ema_15m'] = ta.EMA(informative_15m, timeperiod=50)

        dataframe['hma_50'] = qtpylib.hull_moving_average(dataframe['close'], window=50)

        dataframe = merge_informative_pair(dataframe, informative, self.timeframe, inf_tf, ffill=True)
        dataframe = merge_informative_pair(dataframe, informative_15m, self.timeframe, inf_tf_15m, ffill=True)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[
                ...
                (dataframe['closedelta'] > 0) &    # from BinH
        ...
        # --- VWAP signal (restored from prior strategy) ---
        dataframe.loc[
            (
        ...
        # --- Bounce Reversal at Support signal ---
        dataframe.loc[
            (
                (dataframe['close'] > dataframe['sup_level_1h'] * 1.01) &
                (dataframe['close'] < dataframe['sup_level_1h'] * 1.05) &
                (dataframe['volume'] > dataframe['volume_mean_24'])
            ),
            ['enter_long', 'enter_tag']
        ] = (1, 'bounce_support')

        # --- Range-bound Mean Reversion signal ---
        dataframe.loc[
            (
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        filled_buys = trade.select_filled_orders('buy')
        count_of_buys = len(filled_buys)
        if count_of_buys == 1 and (last_candle['tpct_change_0'] > 0.001) and (last_candle['close'] < last_candle['open']) :
            
                return None
        elif count_of_buys == 2 and (last_candle['tpct_change_0'] > 0.001) and (last_candle['close'] < last_candle['open']) and (last_candle['ema_vwap_diff_50'] < 0.5):
            
                return None
        elif count_of_buys == 3 and (last_candle['tpct_change_0'] > 0.001) and (last_candle['close'] < last_candle['open'])and (last_candle['ema_vwap_diff_50'] < 0.5) :
           
                
                return None
        elif count_of_buys == 4 and (last_candle['tpct_change_0'] > 0.001) and (last_candle['close'] < last_candle['open'])and (last_candle['ema_vwap_diff_50'] < 0.5) and (last_candle['ema_5']) >= (last_candle['ema_10']):
